...
* Descriptive variable names are best for most cases.
    ```
...
* Do not use `for...in` statements with the intent of iterating over a list of numeric keys. Use a traditional for loop instead.
...
* Don't rely on JavaScript's implicit global variables. If you are intending to write to the global scope, export things to `window.*` explicitly instead.
